...
describe 'cumulus_license::license' do
  let(:title) { 'lic' }
  let(:cmd_str) { 'cl-license -i http://10.1.1.1/license' }
  let(:params) { {:src => 'http://10.1.1.1/license' }}
  let(:facts) { { :operatingsystem => 'CumulusLinux' }}
  describe 'using cumulus' do
    it { should contain_exec('install_license').with_command(cmd_str) }
  end
#
